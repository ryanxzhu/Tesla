...
(function(w) {
  ...
  boomr = {
    ...
    utils: {
      ...
      setCookie: function(name, subcookies, max_age) {
        ...
        if (
          ...
          typeof subcookies === "undefined"
        ) {
          BOOMR.debug(
            "Invalid parameters or site domain: " +
              name +
              "/" +
              subcookies +
              "/" +
      ...
      arrayFilter: function(array, predicate) {
        ...
        if (
          !(
            this.isArray(array) ||
            (array && typeof array.length === "number")
          ) ||
      ...
      removeListener: function(el, type, fn) {
        ...
        if (impl.listenerCallbacks.hasOwnProperty(type)) {
          for (var i = 0; i < impl.listenerCallbacks[type].length; i++) {
            if (
              fn === impl.listenerCallbacks[type][i].fn &&
      ...
      measure: function(measureName, startMarkName, endMarkName) {
        ...
        if (
          p &&
          typeof p.measure === "function" &&
          !BOOMR.window.BOOMR_no_mark
        ) {
          p.measure(
            "boomr:" + measureName,
    ...
    init: function(config) {
      ...
      if (
        (!impl.handlers_attached && !config.wait) ||
        (impl.handlers_attached && impl.waiting_for_config)
      ) {
        // The developer can override onload by setting autorun to false
        if (
          !impl.onloadfired &&
          (impl.autorun === undefined || impl.autorun !== false)
        ) {
      ...
      (function() {
        ...
        if (visibilityChange !== undefined) {
          ...
          BOOMR.subscribe("visibility_changed", function() {
            ...
            if (
              impl.lastVisibilityState === "prerender" &&
              visState !== "prerender"
            ) {
    ...
    hasBrowserOnloadFired: function() {
      ...
      return (
        (d.readyState && d.readyState === "complete") ||
    ...
    addError: function BOOMR_addError(err, src, extra) {
      ...
      BOOMR.error(
        "Boomerang caught error: " + err + ", src: " + src + ", extra: " + extra
      );
    ...
    readyToSend: function() {
      ...
      for (plugin in this.plugins) {
        if (this.plugins.hasOwnProperty(plugin)) {
          ...
          if (
            typeof this.plugins[plugin].readyToSend === "function" &&
    ...
    responseEnd: function(name, t_start, data, t_end) {
      ...
      if (!BOOMR.readyToSend()) {
        BOOMR.debug(
          "Attempted to call responseEnd before all plugins were Ready to Send, trying again..."
        );
      ...
      if (typeof name === "object") {
        if (!name.url) {
          BOOMR.debug(
            "BOOMR.responseEnd: First argument must have a url property if it's an object"
          );
          return;
        }
    ...
    real_sendBeacon: function() {
      ...
      var isPageLoad =
        typeof impl.vars["http.initiator"] === "undefined" || isSPA;
      ...
      if (BOOMR.session.enabled) {
        impl.vars["rt.si"] =
          BOOMR.session.ID +
          "-" +
          Math.round(BOOMR.session.start / 1000).toString(36);
    ...
    sendBeaconData: function(data) {
      ...
      if (useImg) {
        ...
        xhr = new (BOOMR.window.orig_XMLHttpRequest ||
        ...
        if (iterVars.hasOwnProperty(name) && vars.hasOwnProperty(name)) {
          url.push(
            this.getUriEncodedVar(
              name,
              typeof vars[name] === "undefined" ? "" : vars[name]
            )
          );
  ...
  (function() {
    ...
    BOOMR.checkWindowOverrides = function(win) {
      ...
      for (var objectIndex = 0; objectIndex < objects.length; objectIndex++) {
        ...
        if (
          freshWindow[objectKey] &&
          (typeof freshWindow[objectKey] === "object" ||
            typeof freshWindow[objectKey] === "function")
        ) {
          ...
          if (freshWindow[objectKey].prototype) {
            propertyNames = Object.getOwnPropertyNames(
              freshWindow[objectKey].prototype
            );
            for (var i = 0; i < propertyNames.length; i++) {
              checkWindowObject(
                [objectKey, "prototype", propertyNames[i]].join(".")
              );
            }
          }
        }
    ...
    if (
      BOOMR.utils.getQueryParamValue("overridden") === "true" &&
      w &&
      w.Object &&
